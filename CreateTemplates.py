...
def validate_atoms_order(ligand_block: cif.Block, input_atoms: tuple[str], atoms_count: int) -> \
    ...
    if len(atom_bonds_filtered) != atoms_count:
        logging.error(f"Received: {input_atoms}{os.linesep}Found only: {atom_bonds_filtered}{os.linesep}{ligand_block.find_value('_chem_comp.id')}")
        return None
        ...
        with open(atom_names_filename, 'w') as atoms:
            for ligand_name, atom_names in unique_lines:
                ...
                if ligand_block is None:
                    logging.error(f"The block {ligand_name} was not found in {DEFAULT_DICT_NAME}. Skipping...{os.linesep}")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for conf in result_dict.keys():
            ...
            while lines_count > 1:
                ...
                try:
                    run_site_binder(paths_to_pdbs_for_curr_conf, SB_AVG_DEFAULT_DIR, tmp)
                except Exception:
