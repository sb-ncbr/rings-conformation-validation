...
# def main():
#     parser = argparse.ArgumentParser(description="Determines electron density coverage of a cycle. By default, "
#                                                  "uses trilinear interpolation to infer the electron density value, "
#                                                  "and considers an atom to be covered by the electron density when "
#                                                  "the corresponding intensity is MORE than the threshold for the "
#                                                  "isosurface (1.5 sigma)")
#     parser.add_argument('-s',
#                         action='store_true', help='Simple mode - output is two numbers: first is the number of '
#                                                   'covered atoms, the second is the total number of atoms in a '
#                                                   'cycle')
#     parser.add_argument('-d',
#                         action='store_true', help='Detailed mode - output a sequence of paried values, where '
#                                                   'the first member of each pair is a serial atom number, '
#                                                   'and the second member is "y" - if atom is covered, '
#                                                   'and "n" - if it is not.')
#     parser.add_argument('-m', '--more_or_equal',
#                         action='store_true', help='Atom is considered to be covered by the '
#                                                   'electron density when the corresponding '
#                                                   'intensity is MORE OR EQUAL to the '
#                                                   'threshold for the isosurface')
#     parser.add_argument('-c', '--closest_voxel',
#                         action='store_true', help='Instead of trilinear interpolation, the '
#                                                   'intensity of the closest voxel is used')
#
#     parser.add_argument('input_cycle_pdb',
#                         type=str, help='Input PDB file with coordinates of a cycle, produced by '
#                                        'PatternQuery')
#     parser.add_argument('input_density_ccp4',
#                         type=str, help='Input electron density file for the corresponding protein structure from the '
#                                        'PDB in CCP4 format')
#
#     args = parser.parse_args()
#     print(f"Arguments in electron densicty script are: {args} ")
