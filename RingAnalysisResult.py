...
def addResolution(ring_type, RMSD_data):
    file_information_path = 'PDB_INFORMATION.csv'
...
def addElDensity(ring_type, xray_merged_resolution):
    base_dir = "electron_density_coverage_analysis"
    file2_path = os.path.join(base_dir, "output", f'{ring_type}_params__analysis_output.csv') 
...
def Summary(ring_type, merged_coverage):
    ...
    percentage_resolution_2_or_less_coverage_x = (stats_resolution_2_or_less_coverage_x / total_values_resolution_2_or_less_coverage_x) * 100
    ...
    stats_resolution_2_or_less_coverage_x_df.loc[len(stats_resolution_2_or_less_coverage_x_df)] = ['Total', total_values_resolution_2_or_less_coverage_x, 100]
    stats_resolution_2_or_less_coverage_x_df.to_csv(os.path.join(output_folder, Sheet_1), sep=' ', index=False, float_format='%.2f')
...
if __name__ == '__main__':
    ...
    # Call addResolution
    resolution_result = addResolution(args.ring, rmsd_result)
