...
def process_all_ligands(ring: Ring, main_dir: Path):
    result = {}
    excluded = []

    basedir = main_dir / ring.name.lower() / "filtered_ligands"
    ...
    outputpath = main_dir / ring.name.lower() / "hr_analysis_output"
    Path(outputpath).mkdir(parents=True, exist_ok=True)
    with open(outputpath / "output_HR.json", "w") as f:
        json.dump(result, f, indent=4)

    logger.info(f"HR angle extraction complete. {len(result)} succeeded, {len(excluded)} failed.")
    if excluded:
...
def calc_HR_for_all_rings(main_dir: Path):

    for ring in Ring:
        process_all_ligands(ring, main_dir)