...
def compare_HR(main_dir: Path):

    for ring in Ring:
        #Load standard conformations HR angles
        json_path = Path('workflow') / 'analysis' / 'resources' / 'standardHR' / f"{ring.name.lower()}.json"
        with open(json_path) as f:
            standard_HRs = json.load(f)

        #Load all ligand HR data
        hr_analysis_output = main_dir / ring.name.lower() / "hr_analysis_output"
